...
pub fn loadContext(allocator: std.mem.Allocator, root_path: []const u8) !model.Context {
    ...
    try walk(allocator, &ctx, &pending, root_path, "");
...
fn parseOrgFile(allocator: std.mem.Allocator, ctx: *model.Context, pending: *std.array_list.Managed(PendingLink), abs_root: []const u8, rel: []const u8) !void {
    ...
    const file_id = try std.fmt.bufPrint(&file_id_buf, "file:{s}", .{rel});
    const file_title = extractTitle(bytes) orelse model.basename(rel);
    _ = try ctx.addObject(.{ .id = file_id, .kind = inferFileKind(rel), .title = file_title, .path = rel, .line = 1, .tags = extractTags(bytes) orelse namespaceTags(rel, inferFileKind(rel)), .preview = "org file" });
    ...
    while (lines.next()) |line_raw| {
        ...
        if (std.mem.startsWith(u8, std.mem.trim(u8, line, " \t"), ":ID:")) {
